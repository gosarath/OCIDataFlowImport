...
object uploadProductGroupData {
  ...
  def main(args: Array[String]) {
    ...
    try {
      ...
      val productFilePath = "oci://ProductGroup@" + ociNameSpace + "/ProductGroup_USKO.csv";
      
      val startTime = Calendar.getInstance.getTime;
      println("Start Time : " + startTime);
      
      val spark = SparkSession.builder()
		              .appName("uploadProductGroupData")
		              .getOrCreate();
		              
		  val ssparkContext = spark.sparkContext;
		  val sparkContext = new JavaSparkContext(ssparkContext);
		  
		  println("Number of records in input file = " + fileRecordCount(sparkContext, productFilePath));
      ...
      df.registerTempTable("QSC_PROD_GROUPS_TL")
      val renameInputDF = sqlContext.sql("select ProdGroupId as PROD_GROUP_ID,Language as LANGUAGE,SourceLang as SOURCE_LANG,ProdGroupName as PROD_GROUP_NAME,AdminProdGroupName as ADMIN_PROD_GRP_NAME from QSC_PROD_GROUPS_TL ");
      val unique_df = renameInputDF.withColumn("CREATION_DATE", lit("01-JAN-21 12.00.00.000000000 AM"))
        .withColumn("LAST_UPDATE_DATE", lit("01-JAN-21 12.00.00.000000000 AM"))
        .withColumn("CREATED_BY", lit("GS1"))
        .withColumn("LAST_UPDATED_BY", lit("GS1"));
      
      println("Final ProductGroup Schema to import ");
      ...
      //unique_df.coalesce(numOfPartitions.toInt).foreachPartition(partition => {
        ...
        options += ("dbtable" -> "QSC_PROD_GROUPS_TL")
