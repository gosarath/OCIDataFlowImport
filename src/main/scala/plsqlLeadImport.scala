...
*/
      ...
      val invalidDF = goodRowsDF.filter("LeadNumber is NULL OR CustomerId is NULL OR PrimaryContactId is NULL OR LeadName is NULL")
      ...
      val validRowsDF = df.filter("_corrupt_record is NULL AND LeadNumber is not NULL AND CustomerId is not NULL AND PrimaryContactId is not NULL AND LeadName is not NULL ")
            .select("LeadNumber", "CustomerId", "PrimaryContactId","LeadName", "DealSize", "City", "State", "Country","BudgetAmount")
      validRowsDF.cache()
      println("Cleaning DB");
      // End - Validate
      ...
      leadBaseTableUpload(batchId.toInt, sqlContext,ssparkContext,walletName, srcOciPathStr, user, pwd,importSucessFilePath)
  ...
  def leadBaseTableUpload(batchId: Int,sqlContext: SQLContext,sc: SparkContext,walletName: String, srcOciPathStr: String,user: String,pwd: String,importSucessFilePath: String): String =
  {
...
//"                        update zca_import_data_int set import_status_code = 'E' where batch_id = ? and import_status_code = 'I' and pk1 is not null and pk1 not in (select lead_id from mkl_lm_leads);"+"\n" +
